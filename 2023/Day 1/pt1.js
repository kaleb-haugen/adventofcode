...
valuesArray = valuesArray.map(
  arrayObject => arrayObject.split(``).filter(Number)
);

// Concat first and last numbers in sub arrays
valuesArray = valuesArray.map(
  arrayObject => arrayObject.concat(arrayObject[0] + arrayObject[arrayObject.length -1])
);

// Update array to be only concatenated values
valuesArray = valuesArray.map(
  arrayObject => arrayObject[arrayObject.length -1]
).filter(Number);


// Sum all values in array
var sum = valuesArray.reduce((accumulator, currentValue) => {
