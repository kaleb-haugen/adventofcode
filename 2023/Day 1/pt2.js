...
const calculateSum = (result, item) => {
    ...
    const patMatches = item.toLowerCase().matchAll(/(?=((\d)|one|two|three|four|five|six|seven|eight|nine))/gm);

    // Array to store the matched values
    const hits = [];

    // Collect matched values in the hits array
    for (const match of patMatches) {
        hits.push(match[1]);
    }

    const firstNum = hits[0];
    const lastNum = hits[hits.length-1];

    // If there are matched values, convert and add them to the result
    if (hits.length) {
        const converted = `${isNaN(parseInt(firstNum)) ? numberMapper[firstNum] : firstNum}${isNaN(parseInt(lastNum)) ? numberMapper[lastNum] : lastNum}`;
...
const sum = valuesArray.trim().split(/\n/gm).reduce(calculateSum, 0);
